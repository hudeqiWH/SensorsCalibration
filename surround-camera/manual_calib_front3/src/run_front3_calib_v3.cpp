/*
 * Front 3 Cameras Manual Calibration - Version 3
 * FL(Fisheye) + F(Ocam) + FR(Fisheye) with Front Wall Projection
 * 
 * Changes from v2:
 * - Fixed image flip (removed FlipY)
 * - Added view mode toggle (Stitching / Single Camera)
 * - Improved initial overlap visualization
 */
...
void projectCameraToWall(FrontCameraConfig &cam) {
    ...
    for (int v = 0; v < output_height; v++) {
        for (int u = 0; u < output_width; u++) {
            ...
            // Note: v is from top to bottom in image, but wall_y is from top to bottom
            // So we need to flip v when computing wall_y
            double wall_x = wall_x_min + (u + 0.5) * meter_per_pixel;
            double wall_y = wall_y_min + (output_height - 1 - v + 0.5) * meter_per_pixel;  // Flip v
            
            // Transform wall point to camera coordinates
            // Body frame: x=forward, y=left, z=down
            // Wall is at x=wall_distance, with y=left-right, z=up-down
            Eigen::Vector4d wall_pt_body;
            wall_pt_body << wall_distance, wall_x, -wall_y, 1.0;  // z=-wall_y because wall_y is up in body frame
...
// Get display image based on view mode
Mat getDisplayImage() {
    if (view_mode == 0) {
        return generateFrontStitchingView();
    } else {
        return cameras[view_mode - 1].img_projected.clone();
    }
}
...
int main(int argc, char **argv) {
    if (argc < 4) {
        cout << "Usage: ./run_front3_calib_v3 <image_path> <intrinsic_path> <extrinsic_json>" << endl;
        cout << "\nExample:" << endl;
        cout << "  ./run_front3_calib_v3 ./imgs ./intrinsics ./extrinsics.json" << endl;
        return 0;
    }
    ...
    cout << "==========================================" << endl;
    cout << "Front 3 Cameras Manual Calibration (v3)" << endl;
    cout << "==========================================" << endl;
    ...
    cout << "  All projections done" << endl;
    ...
    cout << "==========================================" << endl;
    cout << "\nControls:" << endl;
    cout << "  - Frame 0/1/2: Select camera (FL/F/FR)" << endl;
    cout << "  - View Mode buttons: Switch between Stitching/Single view" << endl;
    cout << "  - +/- x/y/z deg/trans: Adjust extrinsics" << endl;
    cout << "  - Reset: Reset current camera to initial" << endl;
    cout << "  - Save: Save all calibrations" << endl;
    
    // Setup Pangolin
    int width = 1200, height = 600;
    pangolin::CreateWindowAndBind("Front 3 Cameras Calibration v3", width, height);
