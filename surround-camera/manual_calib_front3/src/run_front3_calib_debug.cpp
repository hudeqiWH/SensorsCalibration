/*
 * Front 3 Cameras Manual Calibration - DEBUG VERSION
 * Adds debug output to diagnose projection errors
 */
...
bool debug_mode = true;  // Enable debug output
...
void loadFisheyeIntrinsic(FrontCameraConfig &cam, const string &filename) {
    ...
    if (debug_mode) {
        cout << "\n[DEBUG] Loaded fisheye " << cam.name << ":" << endl;
        cout << "  Image size: " << cam.width << "x" << cam.height << endl;
        cout << "  Camera matrix K:" << endl;
        cout << "    [" << cam.camera_matrix.at<double>(0,0) << ", " 
             << cam.camera_matrix.at<double>(0,1) << ", " << cam.camera_matrix.at<double>(0,2) << "]" << endl;
        cout << "    [" << cam.camera_matrix.at<double>(1,0) << ", " 
             << cam.camera_matrix.at<double>(1,1) << ", " << cam.camera_matrix.at<double>(1,2) << "]" << endl;
        cout << "    [" << cam.camera_matrix.at<double>(2,0) << ", " 
             << cam.camera_matrix.at<double>(2,1) << ", " << cam.camera_matrix.at<double>(2,2) << "]" << endl;
        cout << "  Distortion coeffs: [" << cam.dist_coeffs.at<double>(0,0) << ", "
             << cam.dist_coeffs.at<double>(0,1) << ", " << cam.dist_coeffs.at<double>(0,2) << ", "
             << cam.dist_coeffs.at<double>(0,3) << "]" << endl;
    }
}
...
void loadOcamIntrinsic(FrontCameraConfig &cam, const string &filename) {
    ...
    if (debug_mode) {
        cout << "\n[DEBUG] Loaded Ocam " << cam.name << ":" << endl;
        cout << "  Image size: " << cam.width << "x" << cam.height << endl;
        cout << "  Principal point: (" << cam.ocam.cx << ", " << cam.ocam.cy << ")" << endl;
        cout << "  Affine params: c=" << cam.ocam.c << ", d=" << cam.ocam.d << ", e=" << cam.ocam.e << endl;
    }
}
...
void loadExtrinsics(const string &filename) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (debug_mode) {
            cout << "\n[DEBUG] Loaded " << cameras[i].name << " extrinsics:" << endl;
            cout << "  Rodrigues vector: [" << rvec[0] << ", " << rvec[1] << ", " << rvec[2] << "]" << endl;
            cout << "  Translation (cam in body): [" << translation_vec[0] << ", " 
                 << translation_vec[1] << ", " << translation_vec[2] << "]" << endl;
            cout << "  T_body_to_cam (used for projection):" << endl;
            cout << "    [" << cameras[i].extrinsic(0,0) << ", " << cameras[i].extrinsic(0,1) << ", " 
                 << cameras[i].extrinsic(0,2) << ", " << cameras[i].extrinsic(0,3) << "]" << endl;
            cout << "    [" << cameras[i].extrinsic(1,0) << ", " << cameras[i].extrinsic(1,1) << ", " 
                 << cameras[i].extrinsic(1,2) << ", " << cameras[i].extrinsic(1,3) << "]" << endl;
            cout << "    [" << cameras[i].extrinsic(2,0) << ", " << cameras[i].extrinsic(2,1) << ", " 
                 << cameras[i].extrinsic(2,2) << ", " << cameras[i].extrinsic(2,3) << "]" << endl;
        }
    }
}

// Debug projection at sample points
void debugProjection(FrontCameraConfig &cam) {
    if (!debug_mode) return;
    
    cout << "\n[DEBUG] Projection test for " << cam.name << ":" << endl;
    
    // Test points on the wall
    double test_points[][3] = {
        {wall_distance, 0, 0},        // Center
        {wall_distance, 2, 0},        // Left 2m
        {wall_distance, -2, 0},       // Right 2m
        {wall_distance, 0, 1},        // Up 1m
        {wall_distance, 0, -1},       // Down 1m
    };
    const char* point_names[] = {"Center", "Left 2m", "Right 2m", "Up 1m", "Down 1m"};
    
    for (int i = 0; i < 5; i++) {
        Eigen::Vector4d wall_pt_body;
        wall_pt_body << test_points[i][0], test_points[i][1], test_points[i][2], 1.0;
        
        Eigen::Vector4d wall_pt_cam = cam.extrinsic * wall_pt_body;
        
        cout << "  " << point_names[i] << " (body): [" << test_points[i][0] << ", " 
             << test_points[i][1] << ", " << test_points[i][2] << "]" << endl;
        cout << "    -> Cam coords: [" << wall_pt_cam(0) << ", " << wall_pt_cam(1) << ", " << wall_pt_cam(2) << "]" << endl;
        
        if (wall_pt_cam(2) > 0) {
            if (cam.type == FISHEYE_OPENCV) {
                double xn = wall_pt_cam(0) / wall_pt_cam(2);
                double yn = wall_pt_cam(1) / wall_pt_cam(2);
                double r = sqrt(xn * xn + yn * yn);
                double theta = atan(r);
                
                Mat D = cam.dist_coeffs;
                double theta2 = theta * theta;
                double theta4 = theta2 * theta2;
                double theta6 = theta4 * theta2;
                double theta8 = theta6 * theta2;
                double theta_d = theta * (1 + D.at<double>(0,0) * theta2 + D.at<double>(0,1) * theta4 
                                        + D.at<double>(0,2) * theta6 + D.at<double>(0,3) * theta8);
                
                if (r > 1e-10) {
                    double scale = theta_d / r;
                    double xd = xn * scale;
                    double yd = yn * scale;
                    double u = cam.camera_matrix.at<double>(0, 0) * xd + cam.camera_matrix.at<double>(0, 2);
                    double v = cam.camera_matrix.at<double>(1, 1) * yd + cam.camera_matrix.at<double>(1, 2);
                    cout << "    -> Image: (" << u << ", " << v << ")" << endl;
                }
            } else {  // OCAM
                ...
                if (norm > 1e-14) {
                    ...
                    double u = uu + vv * cam.ocam.e + cam.ocam.cx;
                    double v = uu * cam.ocam.d + vv * cam.ocam.c + cam.ocam.cy;
                    cout << "    -> Image: (" << u << ", " << v << ")" << endl;
                }
            }
        } else {
            cout << "    -> Behind camera (z=" << wall_pt_cam(2) << ")" << endl;
        }
    }
}
...
void projectCameraToWall(FrontCameraConfig &cam) {
    ...
    for (int v = 0; v < output_height; v++) {
        for (int u = 0; u < output_width; u++) {
            // Map image pixel to wall coordinates
...
void applyTailMask(Mat& img, int cam_idx) {
    ...
    if (cam_idx == 0) {
        for (int x = 0; x < width; x++) {
            if (x >= width - tail_size) mask.col(x).setTo(255);
        ...
        for (int x = 0; x < width; x++) {
            if (x >= center_start && x <= center_end) mask.col(x).setTo(255);
        ...
        for (int x = 0; x < width; x++) {
            if (x < tail_size) mask.col(x).setTo(255);
...
int main(int argc, char **argv) {
    if (argc < 4) {
        cout << "Usage: ./run_front3_calib_debug <image_path> <intrinsic_path> <extrinsic_json>" << endl;
        cout << "\nThis DEBUG version prints detailed information about:" << endl;
        cout << "  - Camera intrinsics (K matrix, distortion coeffs)" << endl;
        cout << "  - Extrinsics (rotation, translation, T_body_to_cam)" << endl;
        cout << "  - Projection test points (wall -> camera -> image)" << endl;
        return 0;
    }
    
    string image_path = argv[1], intrinsic_path = argv[2], extrinsic_file = argv[3];
    cout << "========================================" << endl;
    cout << "Front 3 Cameras Calibration (DEBUG)" << endl;
    cout << "========================================" << endl;
    ...
    // Debug projection for each camera
    for (int i = 0; i < 3; i++) debugProjection(cameras[i]);
    ...
    pangolin::CreateWindowAndBind("Front 3 Cameras Calibration - DEBUG", width, height);
    ...
    while (!pangolin::ShouldQuit()) {
        ...
        if (pangolin::Pushed(viewStitching)) { view_mode = 0; viewFL = false; viewF = false; viewFR = false; display_image = getDisplayImage(); }
        if (pangolin::Pushed(viewFL)) { view_mode = 1; viewStitching = false; viewF = false; viewFR = false; display_image = getDisplayImage(); }
        if (pangolin::Pushed(viewF)) { view_mode = 2; viewStitching = false; viewFL = false; viewFR = false; display_image = getDisplayImage(); }
        if (pangolin::Pushed(viewFR)) { view_mode = 3; viewStitching = false; viewFL = false; viewF = false; display_image = getDisplayImage(); }
        if (pangolin::Pushed(toggleTail)) { use_tail_effect = !use_tail_effect; if (view_mode == 0) display_image = getDisplayImage(); }
        
        if (frameStep.GuiChanged()) { cali_frame = frameStep.Get(); CalibrationScaleChange(cali_frame); }
        if (degreeStep.GuiChanged()) { cali_scale_degree_ = degreeStep.Get(); CalibrationScaleChange(cali_frame); }
        if (tStep.GuiChanged()) { cali_scale_trans_ = tStep.Get() / 100.0; CalibrationScaleChange(cali_frame); }
        if (wallDist.GuiChanged()) { wall_distance = wallDist.Get(); updateAllProjections(); display_image = getDisplayImage(); }
        ...
        if (pangolin::Pushed(saveButton)) { imwrite("front3_stitched.png", generateFrontStitchingView()); }
